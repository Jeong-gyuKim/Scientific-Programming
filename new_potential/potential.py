...
init(int(datetime.now().timestamp()), myrank)
N = 10**9 #MC steps
l = 25 #total radius to concern
n = 125 #bin number

part_size = N // world_size
N = part_size * world_size

wos = sampling(get_wos,part_size,l,n)
wop = sampling(invCDF,part_size,l,n)
gathered_wos = comm.gather(wos, root=0)
gathered_wop = comm.gather(wop, root=0)

if (myrank == 0):
    final_wos = sum(np.array(gathered_wos))
    final_wop = sum(np.array(gathered_wop))
    print("world size:", world_size)
    print("N:", N)
    print("wos:", final_wos, sum(final_wos))
    print("wop:", final_wop, sum(final_wop))
